...
const jsonprovider = new JsonRpcProvider(`https://eth-mainnet.g.alchemy.com/v2/${INFRURA_PROJECT_ID}`);
export const provider = jsonprovider;
// function to get the amount of COMP received for a given amount of USDC
export async function getAmountOut(amountIn) {
    try {
        ...
        const trade = await Trade.bestTradeExactIn([usdcCompPair], new TokenAmount(USDC, BigInt(amountIn)), COMP, {
            maxNumResults: 1,
            maxHops: 3,
        });
...
export async function getPath(amountIn) {
    // Fetch the pair instance for USDC-COMP
    const usdcCompPair = await Fetcher.fetchPairData(USDC, COMP);
    // get the best trade route for the USDC -> COMP trade
    const [trade] = await Trade.bestTradeExactIn([usdcCompPair], new TokenAmount(USDC, BigInt(amountIn)), COMP, { maxNumResults: 1 });
