...
export const provider = new JsonRpcProvider(`https://eth-mainnet.g.alchemy.com/v2/${INFRURA_PROJECT_ID}`);
// function to get the amount of COMP received for a given amount of USDC
export function getAmountOut(amountIn) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            // Fetch the pair instance for USDC-COMP
            const usdcCompPair = yield Fetcher.fetchPairData(USDC, COMP, provider);
            // create a trade object to represent the swap
            const trade = yield Trade.bestTradeExactIn([usdcCompPair], new TokenAmount(USDC, BigInt(amountIn)), COMP, {
                maxNumResults: 1,
                maxHops: 3,
            });
...
export function getPath(amountIn) {
    return __awaiter(this, void 0, void 0, function* () {
        // Fetch the pair instance for USDC-COMP
        const usdcCompPair = yield Fetcher.fetchPairData(USDC, COMP);
        // get the best trade route for the USDC -> COMP trade
        const [trade] = yield Trade.bestTradeExactIn([usdcCompPair], new TokenAmount(USDC, BigInt(amountIn)), COMP, { maxNumResults: 1 });
